...
- *loader.xml*: a **partial** Riivolution XML that includes the *loader.bin* data and the game hooks to invoke it.

Copy the contents of *loader.xml* into your Riivolution XML's `<patch>` tag.

Since the XML line containing the *loader.bin* data is very long, a common practice is to modify it to load from an external file instead. To do this, put *loader.bin* somewhere in your Riivolution patch files, then replace that XML line with `<memory offset="0x80001900" valuefile="Code/loader.bin" />`, adjusting the path string as needed.
